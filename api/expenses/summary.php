 ...
 * GET ?month=YYYY-MM
 * Returns:
 *   total_spent, personal_total, group_total,
 *   by_category [{name, total}], by_day [{day, total}],
...
// --- By day ---
$sql = "SELECT DAY(e.expense_date) AS day_num, COALESCE(SUM(e.amount), 0) AS total
        FROM expenses e
        WHERE e.expense_date BETWEEN ? AND ?
          AND (
              (e.type = 'personal' AND e.user_id = ?)
...
$res = $stmt->get_result();
while ($r = $res->fetch_assoc()) {
    $byDay[] = ['day' => (int) $r['day_num'], 'total' => (float) $r['total']];
}
$stmt->close();
