...
// Global CSRF token map: cookie => csrfToken
$csrfTokens = [];
 ...
 */
function http(string $method, string $url, array $data = [], string $cookie = '', array $extraHeaders = []): array {
    $ch = curl_init();
    ...
    return ['status' => $httpCode, 'body' => $body, 'json' => $json, 'headers' => $respHeaders];
}

/** Extract PHPSESSID from Set-Cookie headers (use last cookie for session_regenerate_id support) */
function extractSession(string $headers): string {
    if (preg_match_all('/Set-Cookie:\s*PHPSESSID=([^;\s]+)/i', $headers, $m)) {
        return 'PHPSESSID=' . end($m[1]);
    }
    return '';
}

/** Extract csrf_token from Set-Cookie headers */
function extractCsrfToken(string $headers): string {
    if (preg_match_all('/Set-Cookie:\s*csrf_token=([^;\s]+)/i', $headers, $m)) {
        return end($m[1]);
    }
    return '';
}

/** POST to API, return json (auto-attaches cookie + CSRF header + CSRF cookie) */
function apiPost(string $path, array $data, string $cookie, string $csrfToken = ''): array {
    global $baseUrl, $csrfTokens;
    // Auto-resolve CSRF token from global map if not provided
    if ($csrfToken === '' && isset($csrfTokens[$cookie])) {
        $csrfToken = $csrfTokens[$cookie];
    }
    $headers = [];
    $fullCookie = $cookie;
    if ($csrfToken !== '') {
        $headers[] = "X-CSRF-Token: $csrfToken";
        $fullCookie .= "; csrf_token=$csrfToken";
    }
    $r = http('POST', "$baseUrl/api/$path", $data, $fullCookie, $headers);
...
function cleanupTestData(mysqli $conn) {
    ...
    $pattern = '%@exptest.local';
    $stmt = $conn->prepare("DELETE FROM users WHERE email LIKE ?");
    $stmt->bind_param('s', $pattern);
    $stmt->execute();
    $stmt->close();

    // Clear rate limits so tests aren't throttled
    $conn->query("TRUNCATE TABLE rate_limits");

    // Clear health metrics so orchestrator isn't overloaded
    require_once __DIR__ . '/../api/services/MetricsService.php';
    MetricsService::reset();

    // Clear ephemeral notification data files
    $notifDir = __DIR__ . '/../data/notifications';
    if (is_dir($notifDir)) {
        foreach (glob("$notifDir/*.json") as $f) @unlink($f);
        foreach (glob("$notifDir/*.lock") as $f) @unlink($f);
    }
}
...
foreach ($tables as $t) {
    // Table names can't be parameterized; validated against hardcoded whitelist above
    $stmt = $conn->prepare("SELECT 1 FROM `$t` LIMIT 0");
    $ok = $stmt !== false;
    if ($ok) { $stmt->execute(); $stmt->close(); }
    test("Table '$t' exists", $ok);
}
...
$csrfA = extractCsrfToken($r['headers']);
